...
data = []
with open("backend/seated_cable_rows.csv") as f:
    reader = csv.reader(f, dialect='unix')
    for row in reader:
        number = float(row[0])
        print(row)
        lists = [eval(item) for item in row[1:]]
        data.append(lists)

# ['excel', 'excel-tab', 'unix']

# df = pd.read_csv("backend/seated_cable_rows.csv", 
#                  converters={col: ast.literal_eval for col in array_columns})

df = pd.DataFrame(data, columns=array_columns)
                #  converters={col: ast.literal_eval for col in array_columns})
